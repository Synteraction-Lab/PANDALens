...
class App:
    def __init__(self, user_id="u01"):
        ...
        task_description = 'Human: You are my intelligent diary notebook. ' \
                           ...
                           'you need to generate a interesrting diary for my whole day. I will use the diary for later twitter or blog writing.\n ' \
    ...
    def on_press(self, key):
        if (not self.is_recording) and str(key).strip("''").lower() == SUMMARIZE_KEY:
            command_type = "Summarize today's all the items."
        ...
        if str(key).strip("''").lower() == NEW_ITEM_KEY or str(key).strip("''").lower() == REVISE_KEY:
            if not self.is_recording:
                ...
                elif str(key).strip("''").lower() == SUMMARIZE_KEY:
                    command_type = "Summarize today's all the items."
